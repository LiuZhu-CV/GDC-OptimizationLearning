...
class Energy_Choose_Layer2(Layer):
    ...
    def call(self, inputs):
        ...
        # return [(1-signal) * x + signal * x_old, Energy_now, Energy_old]
        # return [x, Energy_now, Energy_old]
        return [(1-signal) * x + signal * x_old, (1-signal) * Energy_now + signal * Energy_old]
...
def soft_thresholding2(x,theta):
    return tf.sign(x) * tf.maximum(np.abs(x) - theta, 0)
...
class Proximal_Layer(Layer):
    ...
    def call(self, inputs):
        ...
        # soft thresholding in idct2 domain:

        dct2d = get_dct_model()
        dct_x = dct2d(x)
        dct_x = soft_thresholding2(dct_x, self.thres)
...
class GradientDescentLayer(Layer):
    def __init__(self,iter,dis_prior_weight,**kwargs):
        self.t=0.0006
        # Attention -> fix the wrong part: the response of clear image is bigger than fake ones(blur & noise)
        self.beta=0.00006   #dis_prior_weight
        # self.beta=-0.04
        self.epi=0.0001*4*(iter+1)
